...
@customElement('art-password-input')
export class ArtPasswordInput extends BaseArtInput implements IArtPasswordInput {
  ...
  protected renderInput(): TemplateResult | null {
    return html`
      <div class="relative inline-block">
        <input
          data-testid="art-password-input"
          type="password"
          id="${this.name}"
          class="art-control"
          name="${this.name}"
...
declare global {
  interface HTMLElementTagNameMap {
    'art-password-input': ArtPasswordInput;
  }
}
